...
export const calculateWin = (winningNumber: number, bets: any) => {
...
const sendGameStage = (gameStage: GameLoop) => {
...
timer.addEventListener("secondsUpdated", function (e: any) {
    ...
    if (currentTime === 1) {
        ...
        sendGameStage(GameLoop.PLACE_BET);
    } else if (currentTime === 25) {
        console.log("NO MORE BETS");
        winningNumber = getRandomNumber(0, 36);
        console.log(winningNumber);
        sendGameStage(GameLoop.SPIN_WHEEL);
    } else if (currentTime === 30) {
        calculateWin(winningNumber, bet.bets);
        console.log(`Win: ${win}`);
        sendGameStage(GameLoop.WINNER);
    } else if (currentTime === 35) {
        sendGameStage(GameLoop.EMPTY_BOARD);
    }
    return;
});
